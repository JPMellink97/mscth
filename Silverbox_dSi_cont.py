...
fit_sys_ss_enc = deepSI.fit_systems.SS_encoder_deriv_general(nx=2, na=20, nb=20, tau=tau)
...
fit_sys_ss_enc.fit(train_sys_data=train_enc, val_sys_data=val_enc, \
                   epochs=500, batch_size=256, loss_kwargs={'nf':80}) #nf is T in paper
...
plt.show()
plt.savefig('test_res_sim_v2.png')
plt.close()
